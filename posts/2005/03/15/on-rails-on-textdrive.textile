...
like Typo, implements only %(=caps)XML%-RPC for posting/editing.
unlike Typo, only implements the MetaWeblog %(=caps)API%, and then via the new Rails ActionWebServices (as ganked from EliteJournal)
fulltext search
%(=caps)RSS 2%.0 and Atom feeds
