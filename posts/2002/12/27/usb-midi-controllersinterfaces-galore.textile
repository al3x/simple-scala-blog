...
"%(=caps)WAMU%":http://www.wamu.org, one to the "ACLU":http://www.aclu.org, and nobody
to the "%(=caps)EFF%":http://www.eff.org. Minus the last one, that made me really happy.
...
%(=caps)USB MIDI% controller":http://www.audiomidi.com/common/cfm/product.cfm?Product_ID=2510. It’s swell: 16 assignable %(=caps)MIDI% controller knobs, perfect for
just about any audio app, and totally %(=caps)USB% bus-powered (that means no wall wart power
supply, yay!). It’s a great solution, but I still want a %(=caps)USB% audio interface with a
...
rightly, to keep things simple) is synthesis. Through creative %(=caps)VST% filtering you can do
...
environment, is the only such professional tool that’s Mac %(=caps)OS X% ready today (albeit in
