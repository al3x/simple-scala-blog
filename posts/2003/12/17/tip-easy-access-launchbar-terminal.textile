...
Here’s a very little but very useful tip for you Mac %(=caps)OS X 10%.3 users who use the fantastic "LaunchBar":http://www.obdev.at/products/launchbar/ utility and frequently connect to remote servers via the Apple-provided Terminal application:
...
Whether or not to run the %(=caps)SSH% process in a shell depends on if you want the window just disappearing when you log out of the remote server (don’t run in a shell) or want it to linger about telling you that it’s done its duty (do run in a shell).
...
You’re now able to open a connection to your most-used remote servers with a mere Command-Space and the first few letters of the server name!  LaunchBar will present you with your Terminal file(s), and simply hitting Return will launch a new Terminal window containing a new %(=caps)SSH% connection to that remote machine.

If you really want to streamline your remote work, consider generating an %(=caps)SSH% keypair and uploading your public key as an authentication token: you’ll be automatically logged in on supported servers!  "Here is":http://www.cs.utk.edu/~england/ssh.html a solid tutorial on this process, and many others can be found.

For more Mac %(=caps)OS X% tips and tricks see "Mac %(=caps)OS X% Hints":http://www.macosxhints.com/.  Enjoy!